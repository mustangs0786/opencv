...
def facial_processing(url):
    ...
    while True:
        # counter += 10
        # if (counter%10==0):
        #     print ("i am in "+str(counter))
        grab , frame=cap.read()

        if grab:
            a = (cap.get(cv2.CAP_PROP_POS_MSEC)/1000)
            Y.append(a)
           # X.append(0)
        ...
        if not grab:
            plt.scatter(X,Y, label='drowsiness', color='green', s=30, marker="*")

            plt.xlabel('eye,Mouth')
            plt.ylabel('time')
        ...
        if rect!=None:
            ...
            cv2.putText(frame, "Eye: {:.2f} MAR{:.2f}".format(eye_aspect_ratio,mar), (10, frame.shape[0]-10),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            if eye_aspect_ratio < EYE_DROWSINESS_THRESHOLD:
                X.append(eye_aspect_ratio)
                ...
                if time.time()-eye_start_time >= EYE_DROWSINESS_INTERVAL:
                    ...
                    alarm_type=0
                    cv2.putText(frame, "YOU ARE SLEEPY...\nPLEASE TAKE A BREAK!", (10, 20),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
if __name__=='__main__':
    ...
    # counter =1
    url = "sample.mp4"
    facial_processing(url)
    # for x in facial_processing(url):
